...
public class DiskCache implements ICache {
    ...
    public DiskCache(Context context) {
        this(context, getDiskCacheDir(context, MyConfig.CACHE_DISK_DIR),
                calculateDiskCacheSize(getDiskCacheDir(context, MyConfig.CACHE_DISK_DIR)));
    }

    public DiskCache(Context context, File diskDir, long diskMaxSize) {
        compile = Pattern.compile(REGEX);
        try {
            cache = DiskLruCache.open(diskDir, 1, 1, diskMaxSize);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void put(String key, Object value) {
        put(key, value != null ? JSON.toJSONString(value) : null);
    }
